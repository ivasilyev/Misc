...
def usage():
    print("\nUsage: " + sys.argv[0] + " -a <str> -b <str> -z <regex> -y <regex> -o <str>" + "\n\n" +
          "-a/--item1 <str> \tFirst file or folder" + "\n" +
          "-b/--item2 <str> \tSecond file or folder" + "\n" +
          "-z/--perl1 <regex> \tRegular expression to search in each filename of the first file" + "\n" +
          "-y/--perl2 <regex> \tRegular expression to search in each filename of the second file" + "\n" +
...
def strings_process(str_list, regex):
    processed_list = []
    for processing_str in str_list:
        try:
            processed_list.append(re.search(regex, processing_str).group(1))
        except (AttributeError, IndexError) as exception:
            processed_list.append(processing_str)
    return processed_list


def compare_lists(list_to_check, main_list, check_regex):
    comparison_table = "Entry\tStatus\n"
    for raw_string in list_to_check:
        try:
            string_to_check = re.search(check_regex, raw_string).group(1)
        except (AttributeError, IndexError) as exception:
...
######################################################
object1, object2, regex1, regex2, outputMask = main()

list1 = parse_some(object1)
list2 = parse_some(object2)

straight_comparison = compare_lists(list1, list2, regex1)
reversed_comparison = compare_lists(list2, list1, regex2)
